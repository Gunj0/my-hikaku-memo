...
本番オリジンは `https://my-hikaku-memo.com/` である。本書で「サイト URL」と呼ぶものはこのオリジンを指し、canonical、OGP、sitemap.xml、robots.txt の絶対 URL はすべてこれを基準に組み立てる。実際の値は Cloudflare Worker の環境変数から解決するため、リポジトリには持たない（「9. 絶対 URL の解決」を参照）。
`www.my-hikaku-memo.com` は未設定で 404 を返すため、共有や被リンクに用いないこと。
...
- 連番であるため URL は総当たりで列挙できる。これは**受容した設計判断**である。閲覧可否は「公開メモである」または「閲覧者が所有者である」で必ず判定しており、列挙しても非公開メモの中身と存在は得られない。外部から観測できるのは累計メモ数と作成順序に限られ、短く共有しやすい URL の利点がこれを上回ると判断した。非公開メモの秘匿性が要件になった場合は、この判断を見直して不透明な識別子へ移行すること。
