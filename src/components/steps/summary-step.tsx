...
import { TrophyIcon, StarIcon, TrendingUpIcon } from 'lucide-react'
...
export function SummaryStep({ products, decisionPoints, scores }: SummaryStepProps) {
  ...
  const calculateWeightedScore = (productId: string, point: DecisionPoint): number => {
                  ...
                  <div className="h-2 bg-secondary rounded-full overflow-hidden">
                    <div
                      className={cn(
                        ...
                        index === 0 && topScore > 0 ? 'bg-primary' : 'bg-muted-foreground/50'
                      )}
      ...
      <div className="p-4 bg-secondary/50 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p className="mt-1">最高スコアの製品が最もあなたの重視ポイントにマッチしています。</p>
          </div>
        </div>
      </div>
    </div>
